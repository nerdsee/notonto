...
public class Lesson {
	...
	public void storeData() {
		...
		try {
			...
			BrainSession session = bs.getBrainSession();
			bs.unloadCategories();
			session.unloadLocalCategories();
	...
	public int getMaxItemsLevel0() {
		...
		return BrainSystem.getBrainSystem().getMaxItemsLevel0();
		...
		if (group == null) {
			try {
				if (ownergroupId > 0)
					...
					long id = BrainSystem.getBrainSystem().getBrainSession().getCurrentUser().getGroupID();
