...
public class Chart {
	...
	public static XYDataset getScoreDataset() {
		...
		BrainSession session = (BrainSession) BrainSystem.accessBeanFromFacesContext("BrainSession",context);

		return session.getScoreDataset();

	}

	public static JDBCXYDataset getDBScoreDataset() {

		JDBCXYDataset xyd = null;
		try {
			User user = BrainSystem.getBrainSystem().getBrainSession().getCurrentUser();
